...
def distribution_plot():
    ...
    phi = [0, 3, 5]
    name = []
    filepath = "Paper_Figure/Distribution/"

    name.append(
        "_Avg_phi=0_regime=3_min_samples=64_tdm=False_lr=[0.0001]_sigma1=1_sigma2=1_gd=True"
    )
    name.append(
        "_Avg_phi=3_regime=3_min_samples=64_tdm=False_lr=[0.0001]_sigma1=1_sigma2=1_gd=True_4tanh0.25x"
    )
    name.append(
        "_Avg_phi=5_regime=3_min_samples=64_tdm=False_lr=[0.0001]_sigma1=1_sigma2=1_gd=True_clipx=4_clipy=4"
    )
    fig1, ax1 = plt.subplots(figsize=(5, 4), tight_layout=True)
    pdf = []
    for ind, name_i in enumerate(name):
        data = io.loadmat(filepath + name_i + "/res.mat")
        power_change = data["Power_Change"]
        pdf.append(io.loadmat(filepath + name_i + "/pdf.mat"))
        ax1.plot(
            power_change[0],
            data["Learned_Capacity"][0],
            label="$\phi=$" + str(phi[ind]),
            linewidth=2,
        )
    ax1.set_xlabel("$P_1$", fontsize=10)
    ax1.set_ylabel("$R_L$", fontsize=10)
    ...
    fig1.savefig(filepath + "comp_rate.png")
    plt.close()
