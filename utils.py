...
def get_max_alphabet_interference(
    ...
    if int_ratio > 0 and int_ratio <= 1:
        #TODO: Only done here
        # X2 does not matter in delta calculation
        if config["x2_fixed"] and config["x2_type"] == "QAM":
            delta = min(
...
def get_interference_alphabet_x_y_complex(
    ...
    if config["x2_fixed"] and config["x2_type"] == "QAM":
        ...
        m = int(np.sqrt(M))
        if m**2 != M:
            raise ValueError("QAM constellation must be square (k must be even)")

        # Create real and imaginary grid points
        ...
        scaling = np.sqrt(power2 / avg_power)
        symbols *= scaling
        real_x2 =  torch.unique(torch.tensor(symbols.real)).reshape(1, -1)
        imag_x2 =  torch.unique(torch.tensor(symbols.imag)).reshape(-1, 1)
    else:
        alphabet_x_2 = torch.arange(-max_x_2, max_x_2 + delta_x2 / 2, delta_x2)
...
def loss_interference(
    ...
    cap_RX2 = torch.zeros_like(cap_RX1)
